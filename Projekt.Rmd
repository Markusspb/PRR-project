...
ui <-dashboardPage(
  ...
  selectInput(
    ...
    label = "Wähle Regression (nur für Plots):",
...
server <- function(input, output) {
  ...
  output$plota <- renderPlot({
    ...
    ggplot(norm_scale, aes(x = Dates, y = Summe_Rented_Bike_Count)) +
      ...
      {if (input$dist=="Sol")geom_line(aes(y = Solar.Radiation_mean ), colour = "yellow", size = 1)}+
      {if (input$dist=="Dew")geom_line(aes(y = Dew.point_mean), colour = "cyan", size = 1)}+
  theme_minimal()
  })
  }
...
server <- function(input, output) {
  ...
  output$plota <- renderPlot({
    ...
    ggplot(norm_scale, aes(x = Dates, y = Summe_Rented_Bike_Count)) +
      ...
      {if ("Sol" %in% input$dist)geom_line(aes(y = Solar.Radiation_mean ), colour = "yellow", size = 0.5)}+
      {if ("Dew" %in% input$dist)geom_line(aes(y = Dew.point_mean), colour = "cyan", size = 0.5)}+
  theme_minimal()
  })
  }
...
server <- function(input, output) {
  ...
  gefilterte_daten <- reactive({
    if (input$season_select == "Alle") {
      byhour<- data %>% group_by(Hour)%>%summarise(Seasons = first(Seasons),Summe_Rented_Bike_Count = sum(Rented.Bike.Count), mean_temp=mean(Temperature..C.),cor_temp=cor(Rented.Bike.Count,Temperature..C.), mean_Humidity=mean(Humidity...,na.rm = TRUE),cor_Humidity=cor(Rented.Bike.Count,Humidity...) ,mean_Wind=mean(Wind.speed..m.s.),cor_Wind=cor(Rented.Bike.Count,Wind.speed..m.s.) , 
Visibility_mean=mean(Visibility..10m.,na.rm =  TRUE),cor_Visibility=cor(Rented.Bike.Count,Visibility..10m.),
Dew.point_mean=mean(Dew.point.temperature..C.),cor_Dewpoint=cor(Rented.Bike.Count, Dew.point.temperature..C.),Solar.Radiation_mean=mean(Solar.Radiation..MJ.m2.), cor_Solar=cor(Rented.Bike.Count,Solar.Radiation..MJ.m2.))%>%ungroup() # ungefilterte Daten
...
Dew.point_mean=mean(Dew.point.temperature..C.),cor_Dewpoint=cor(Rented.Bike.Count, Dew.point.temperature..C.),Solar.Radiation_mean=mean(Solar.Radiation..MJ.m2.), cor_Solar=cor(Rented.Bike.Count,Solar.Radiation..MJ.m2.))
  ...
  output$plot2 <- renderPlot({
    byhour <- gefilterte_daten()
    ggplot(byhour, aes(x = Summe_Rented_Bike_Count, y = mean_Humidity,colour=Seasons)) +
      geom_point() +
      geom_smooth(method = lm, formula = y ~ x, colour = "red")
  })
  ...
  output$plot3 <- renderPlot({
    byhour <- gefilterte_daten()
    ggplot(byhour, aes(x = Summe_Rented_Bike_Count, y = mean_Wind,colour=Seasons)) +
      geom_point() +
      geom_smooth(method = lm, formula = y ~ x, colour = "red")
  })
  ...
  output$plot4 <- renderPlot({
    byhour <- gefilterte_daten()
    ggplot(byhour, aes(x = Summe_Rented_Bike_Count, y = Visibility_mean,colour=Seasons)) +
      geom_point() +
      geom_smooth(method = lm, formula = y ~ x, colour = "red")
  })
  ...
  output$plot5 <- renderPlot({
    byhour <- gefilterte_daten()
    ggplot(byhour, aes(x = Summe_Rented_Bike_Count, y = Dew.point_mean,colour=Seasons)) +
      geom_point() +
      geom_smooth(method = lm, formula = y ~ x, colour = "red")
  })
  ...
  output$plot6 <- renderPlot({
    byhour <- gefilterte_daten()
    ggplot(byhour, aes(x = Summe_Rented_Bike_Count, y = Solar.Radiation_mean,colour=Seasons)) +
      geom_point() +
      geom_smooth(method = lm, formula = y ~ x, colour = "red")
  })
  ...
  output$plot7 <- renderPlot({
    byhour <- gefilterte_daten()
    scale_factor <- max(byhour$Summe_Rented_Bike_Count, na.rm = TRUE) / 50
    ggplot(byhour, aes(x = Dates, y = Summe_Rented_Bike_Count)) +
      geom_bar(stat = "identity") +
      geom_line(aes(y = mean_temp * scale_factor), colour = "red", size = 1) +
      scale_y_continuous(
        name = "Rented Bike Count",
